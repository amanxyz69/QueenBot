...
from QueenBot import DRAGONS
from QueenBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def venom_is_admin(chat_id: int):
    status = False
    venom = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if venom.id == user.id:
